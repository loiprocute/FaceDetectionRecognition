...
//
//     //Check the loaded Mats
//    for (const auto& mat : loadedMatVector) {
...
int main(int argc, char** argv)
{
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (feature.empty()) {
            ...
            images_feature.push_back(feature.clone());
        }
    }
    saveMatVector(images_feature, "mat_vector_data.bin");
    
    // save all_paths which return Mat 
    std::ofstream file("all_paths.txt");
    if (file.is_open()) {
        
        for (const auto& str : all_paths) {
            file << str << std::endl;   
        }
        file.close();
        std::cout << "Strings saved to 'all_paths.txt' file." << std::endl;
    }
    else {
